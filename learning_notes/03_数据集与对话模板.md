...
### 8.2 parse_conversations：抽取 tools + 截断最后一轮
...
### 8.3 __getitem__：返回 messages/tools/gt
...
下一章 `04_训练基础设施（分布式/检查点/采样器）` 会逐行解析 `trainer/trainer_utils.py`，重点把 checkpoint 的保存/恢复字段与张量 dtype（half/AMP）这两件工程细节说清楚。
