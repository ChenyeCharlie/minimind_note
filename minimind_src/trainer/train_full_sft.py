...
# 设置包名并添加上级目录，确保可以导入 model 和 dataset 模块
__package__ = "trainer"
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    """
    全量 SFT (Supervised Fine-Tuning) 单轮训练逻辑。
    与 Pretrain 类似，但数据来自 SFTDataset，其 labels 仅在 Assistant 回复部分有值。
    """
    start_time = time.time()
    last_step = start_step
    
    # 1. 遍历训练数据：input_ids [bs, seq_len], labels [bs, seq_len]
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        # 3. 混合精度前向传播
        with autocast_ctx:
            # 执行 MiniMindForCausalLM 的前向传播，计算交叉熵损失
            # 注意：labels 中为 -100 的位置在 loss 计算时会被自动忽略
            res = model(input_ids, labels=labels)
            # 总损失包含模型输出 Loss 和 MoE 负载均衡辅助 Loss
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # skip 用于断点恢复时跳过当前 Epoch 已经训练过的 step
