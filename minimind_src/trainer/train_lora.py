...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):
    ...
    LoRA 微调单轮训练逻辑。
    Input:
        lora_params: 仅包含 LoRA 层的可训练参数列表。
    """
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        # 3. 混合精度前向传播
        with autocast_ctx:
            # 执行模型计算，由于非 LoRA 参数已冻结，此处主要针对 LoRA 支路计算梯度
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora_name", type=str, default="lora_medical", help="LoRA权重名称")
    ...
    # 调用 model_lora.py 将 LoRA 支路注入到模型线性层
    apply_lora(model)
    ...
    # 核心步骤：冻结所有非 LoRA 参数，仅开启 LoRA 参数的梯度更新
    lora_params = []
    ...
    # ========== 6. 数据与优化器配置 ==========
    # LoRA 训练通常使用 SFT 数据格式
    train_ds = SFTDataset(args.data_path, tokenizer, max_length=args.max_seq_len)
    train_sampler = DistributedSampler(train_ds) if dist.is_initialized() else None
    scaler = torch.cuda.amp.GradScaler(enabled=(args.dtype == 'float16'))
    # 仅将 lora_params 传入优化器
    optimizer = optim.AdamW(lora_params, lr=args.learning_rate)
    
    # ========== 7. 断点状态恢复 ==========
    start_epoch, start_step = 0, 0
    if ckp_data:
        # strict=False 因为 ckp 中可能包含或不包含 LoRA 参数，取决于保存时的状态
